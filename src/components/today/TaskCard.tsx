...
import { TASK_META, isTaskDone } from "@/lib/tasks";
...
export function TaskCard({ task, compact = false }: { task: DailyTask; compact?: boolean }) {
  ...
  return (
    <div className={cn("rounded-lg border bg-card p-4", done && "border-emerald-500/40 bg-emerald-500/5")}>
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 text-right tabular-nums">
          ...
          <span className="text-sm text-muted-foreground"> / {task.target}{meta.unit}</span>
        </div>
      </div>
